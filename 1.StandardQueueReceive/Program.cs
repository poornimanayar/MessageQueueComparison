...
while (true)
{
    ...
    foreach (var message in response.Messages)
    {
        ...
        {
            Console.WriteLine(
                $"Message attribute key : {messageAttribute.Key}, value : {messageAttribute.Value.StringValue}");
        }
        ...
        switch (myAttributeHeader.StringValue)
        {
            case "value1":
                ...
                Console.WriteLine(
                    $"Received message, description: {receivedMessage.Description}, messageId : {message.MessageId}");
                break;
            case "value2":
                //process the message a handler or a service
