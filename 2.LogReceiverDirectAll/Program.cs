...
//bind the queue to the exchange with a routing key info
await channel.QueueBindAsync(queue: queueName, exchange: "direct_logs", routingKey: "info");

//bind the queue to the exchange with a routing key warning
await channel.QueueBindAsync(queue: queueName, exchange: "direct_logs", routingKey: "warning");
