...
{
    var message = new ServiceBusMessage("Message 1 with transactions");
    await sender.SendMessageAsync(message);
...
var queueInfoAfterTransactionComplete = await adminClient.GetQueueRuntimePropertiesAsync(queueName);
Console.WriteLine($"Number of messages in the queue after completing transactions: {queueInfoAfterTransactionComplete.Value.ActiveMessageCount}");
